...
class Orchestrator:
    ...
    async def _run_workflow(self, project_id: str, state: ProjectState = None):
        try:
            ...
            # If the cancellation was explicitly requested (via WebSocket command), mark as stopped.
            # If not (for example server reload/shutdown), leave the DB status unchanged so the
            # startup recovery logic can detect 'running' projects and resume them.
            stop_event = self.get_stop_event(project_id)
            if stop_event.is_set():
                async with get_session() as session:
                    await db_utils.update_project_status(session, UUID(project_id), "stopped")
                # Clear the stop event so future runs aren't affected
                stop_event.clear()
            else:
                LOGGER.info(
                    "Cancellation for project %s does not appear to be a user stop request; leaving status for automatic resume",
                    project_id,
                )
            raise
